...
export const downloadAnalyticsReport = async (req, res, next) => {
  try {
    ...
    const totalMembers = ratioRows.reduce((acc, r) => acc + Number(r.value), 0)
    const active       = ratioRows.find(r => r.status_id === 1)?.value ?? 0
    const expired      = ratioRows.find(r => r.status_id === 2)?.value ?? 0
    const cancelled    = ratioRows.find(r => r.status_id === 3)?.value ?? 0
    ...
    const sumVisits = rows =>
      rows.reduce((acc, r) => acc + Number(r.visits ?? 0), 0)
    ...
    sheet.addRow(["Total Members", Number(totalMembers)])
    sheet.addRow(["Active Members",  Number(active)])
    sheet.addRow(["Expired Members", Number(expired)])
    sheet.addRow(["Cancelled Members", Number(cancelled)])
    sheet.addRow([])
